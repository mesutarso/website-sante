...
function CongoMap({ indicateurs, selectedProvince, onProvinceSelect }: Props) {
    ...
    useEffect(() => {
        if (selectedProvince && selectedProvince !== "all") {
            const provinceData = indicateurs?.find((item: any) => item.province_id === selectedProvince);
            if (provinceData) {
                setTitle(provinceData.province);
            }
        } else {
            setTitle("Toutes les provinces");
        }
    }, [selectedProvince, indicateurs]);
    ...
    const legendThresholds = useMemo(() => {
        if (!indicateurs || indicateurs.length === 0) {
            return { max: 3000, high: 1500, medium: 500, min: 0 };
        }

        const allCases = indicateurs?.map((item: any) => item.cas || 0).filter((cas: number) => cas > 0);
        ...
        const high = Math.ceil(max * 0.7);
        const medium = Math.ceil(max * 0.4);
        const min = Math.min(...allCases);

        return { max, high, medium, min };
    }, [indicateurs]);

    const getRadius = useCallback((cas: number): number => {
        if (!cas || cas === 0) return 3;

        const { max, high, medium } = legendThresholds;
        const baseRadius = 3;
        ...
        if (cas >= high) {
            return Math.min(highRadius + ((cas - high) / (max - high)) * (maxRadius - highRadius), maxRadius);
        } else if (cas >= medium) {
            return Math.min(mediumRadius + ((cas - medium) / (high - medium)) * (highRadius - mediumRadius), highRadius);
        } else {
            return Math.min(baseRadius + (cas / medium) * (mediumRadius - baseRadius), mediumRadius);
        }
    }, [legendThresholds]);

    const getStrokeRadius = useCallback((cas: number): number => {
        const radius = getRadius(cas);
        return radius * 1.3;
    }, [getRadius]);

    const getProvinceData = useCallback((province: string) => {
        const findProvince = indicateurs?.find((item: any) => item.province === province);
        if (findProvince) {
            return {
                nom: province,
                cas: findProvince.cas || 0,
                radius: getRadius(findProvince.cas || 0),
                strokeRadius: getStrokeRadius(findProvince.cas || 0)
            };
        }
        return null;
    }, [indicateurs, getRadius, getStrokeRadius]);

    const getLegendColor = useCallback((level: string) => {
        switch (level) {
            case 'max': return 'text-red-600';
            case 'high': return 'text-red-500';
            case 'medium': return 'text-red-400';
            case 'min': return 'text-red-300';
            default: return 'text-red-500';
        }
    }, []);

    const renderCircleWithTooltip = useCallback((province: string, cx: number, cy: number) => {
        const provinceData = getProvinceData(province);
        const casCount = provinceData?.cas || 0;

        const circleElement = (
            <circle
                cx={cx}
                cy={cy}
                r={provinceData?.radius || 3}
                className={`fill-red circle-stroke animate-pulse stroke-red`}
                style={{ strokeWidth: provinceData?.strokeRadius || 4 }}
            />
        );

        if (casCount > 0) {
            return (
                <Tooltip text={`${casCount} cas confirmés`}>
                    {circleElement}
                </Tooltip>
            );
        }
    ...
    const renderProvinceName = useCallback((province: string, cx: number, cy: number) => {
        const provinceData = getProvinceData(province);
        const casCount = provinceData?.cas || 0;

        if (casCount > 0) {
            const isSelected = title === province;
            return (
                <text
                    x={cx}
                    y={cy + (provinceData?.radius || 3) + 15}
                    className={`text-xs font-medium pointer-events-none ${isSelected ? 'fill-white' : 'fill-gray-700 dark:fill-gray-200'}`}
                    textAnchor="middle"
                >
                    {province}
                </text>
        ...
        return null;
    }, [getProvinceData, title]);

    const homeTranslations = {
        fr: {
            repartionParProvince: "<p>Répartition <br/> par province</p>",
            cas: "Cas",
            legendTitle: "Nombre de cas confirmés",
        },
    };
    ...
    return (
        <div>
            <Card className="map py-3 pb-8">
                <div className='px-5 rounded-md relative my-8 min-h-[400px] md:min-h-[650px] bg-light-gray dark:bg-dark-violet w-full'>

                    <div className='flex justify-between items-center gap-2'>

                        <Button className='bg-[#EBEBEB] hover:bg-[#EBEBEB] dark:bg-[#2D2D3D] dark:hover:bg-[#2D2D3D] min-w-[120px] h-[30px] rounded-md'>
                            <span className="text-center text-black dark:text-white text-xs">
                                {title !== "Toutes les provinces" ? `${title}: ` : ""}{getProvinceData(title)?.cas || total_cas} {homeTranslations.fr.cas}
                            </span>
                        </Button>
                    </div>
                    ...
                    <div className="legend absolute bottom-[-40px] left-5 md:left-5 lg:left-5 space-y-2 flex flex-col justify-start">
                        <div className="mb-2">
                        ...
                        <div className="large-bullet flex items-center gap-2">
                            <div className="bullet-stroke relative bg-red/[19%] w-10 h-10 rounded-full">
                                <Dot size={'10rem'} className={`${getLegendColor('max')} absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10`} />
                            </div>
                            <p className="text-gray-500 dark:text-gray-200 text-xs">
                                &gt; {legendThresholds.high.toLocaleString()} – {legendThresholds.max.toLocaleString()}
                            </p>
                        </div>

                        <div className="medium-bullet flex ml-[8px] items-center gap-2">
                            <div className="bullet-stroke relative bg-red/[19%] w-7 h-7 rounded-full">
                                <Dot size={'7rem'} className={`${getLegendColor('high')} absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10`} />
                            </div>
                            <p className="text-gray-500 dark:text-gray-200 text-xs ml-1">
                                &gt; {legendThresholds.medium.toLocaleString()} – {legendThresholds.high.toLocaleString()}
                            </p>
                        </div>

                        <div className="small-bullet ml-[12px] flex items-center gap-4">
                            <div className="bullet-stroke relative bg-red/[19%] w-5 h-5 rounded-full">
                                <Dot size={'5rem'} className={`${getLegendColor('medium')} absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10`} />
                            </div>
                            <p className="text-gray-500 dark:text-gray-200 text-xs">
                                &gt; {legendThresholds.min.toLocaleString()} – {legendThresholds.medium.toLocaleString()}
                            </p>
                        </div>

                        <div className="smallest-bullet ml-[16px] flex items-center gap-4">
                            <div className="bullet-stroke relative bg-red/[19%] w-3 h-3 rounded-full">
                                <Dot size={'3rem'} className={`${getLegendColor('min')} absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10`} />
                            </div>
                            <p className="text-gray-500 dark:text-gray-200 text-xs">
                                &lt; {legendThresholds.min.toLocaleString()}
                            </p>
                        </div>
                    </div>
                            ...
                            >
                                <g>
                                    <a
                                        ...
                                        id="region-1"
                                        title="Kongo-Central"
                                        onClick={() => handleTitle("Kongo-Central")}
                                        className="map__image__path relative"
                                    >
                                        <g>

                                            <path
                                                className={`${title === "Kongo-Central" ? "fill-active" : "fill"
                                                    }`}
                                                id="CD-BC"
                                            ...
                                            {renderCircleWithTooltip("Kongo-Central", 84.363, 410.892)}
                                            {renderProvinceName("Kongo-Central", 84.363, 410.892)}
                                        </g>

                                    </a>
                                </g>
                                <g>
                                    <a href="#" id="region-2" title="Bas-Uélé" onClick={() => handleTitle("Bas-Uele")}
                                        className="map__image__path relative">
                                        <g>
                                            <path
                                                id="CD-BU"
                                                className={`${title === "Bas-Uele" ? "fill-active" : "fill"
                                                    }`}
                                            ...
                                            ></path>
                                            {renderCircleWithTooltip("Bas-Uele", 484.824, 61.725)}
                                            {renderProvinceName("Bas-Uele", 484.824, 61.725)}
                                        </g>
                                    </a>
                                </g>
                                <g>
                                    <a
                                        ...
                                        id="region-3"
                                        title="Équateur"
                                        onClick={() => handleTitle("Équateur")}
                                    >
                                        <g>
                                            <path
                                                id="CD-EQ"
                                                className={`${title === "Équateur" ? "fill-active" : "fill"
                                                    }`}
                                            ...
                                            ></path>
                                            {renderCircleWithTooltip("Équateur", 255.723, 188.832)}
                                            {renderProvinceName("Équateur", 255.723, 188.832)}
                                        </g>
                                    </a>
                                </g>
                                <g>
                                    <a href="#" id="region-4" title="Haut-Katanga"
                                        onClick={() => handleTitle("Haut-Katanga")}>
                                        <g>
                                            <path
                                                id="CD-HK"
                                                className={`${title === "Haut-Katanga" ? "fill-active" : "fill"
                                                    }`}
                                            ...
                                            ></path>
                                            {renderCircleWithTooltip("Haut-Katanga", 607.279, 576.546)}
                                            {renderProvinceName("Haut-Katanga", 607.279, 576.546)}
                                        </g>
                                    </a>
                                </g>
                                <g>
                                    <a href="#" id="region-5" title="Haut-Lomami"
                                        onClick={() => handleTitle("Haut-Lomami")}>
                                        <g>
                                            <path
                                                id="CD-HL"
                                                className={`${title === "Haut-Lomami" ? "fill-active" : "fill"
                                                    }`}
                                            ...
                                            ></path>
                                            {renderCircleWithTooltip("Haut-Lomami", 504.299, 517.652)}
                                            {renderProvinceName("Haut-Lomami", 504.299, 517.652)}
                                        </g>
                                    </a>
                                </g>
                                <g>
                                    <a href="#" id="region-6" title="Haut-Uele" onClick={() => handleTitle("Haut-Uele")}>
                                        <g>
                                            <path
                                                id="CD-HU"
                                                className={`${title === "Haut-Uele" ? "fill-active" : "fill"
                                                    }`}
                                            ...
                                            ></path>
                                            {renderCircleWithTooltip("Haut-Uele", 654.877, 68.51)}
                                            {renderProvinceName("Haut-Uele", 654.877, 68.51)}

                                        </g>
                                    </a>
                                </g>
                                <g>
                                    <a href="#" id="region-7" title="Ituri" onClick={() => handleTitle("Ituri")}>
                                        <g>
                                            <path
                                                id="CD-IT"
                                                className={`${title === "Ituri" ? "fill-active" : "fill"
                                                    }`}
                                            ...
                                            ></path>
                                            {renderCircleWithTooltip("Ituri", 647.844, 146.086)}
                                            {renderProvinceName("Ituri", 647.844, 146.086)}
                                        </g>
                                    </a>
                                </g>
                                <g>
                                    <a
                                        ...
                                        id="region-8"
                                        title="Kwango"
                                        onClick={() => handleTitle("Kwango")}
                                    >
                                        <g>
                                            <path
                                                id="CD-KG"
                                                className={`${title === "Kwango" ? "fill-active" : "fill"
                                                    }`}
                                            ...
                                            ></path>
                                            {renderCircleWithTooltip("Kwango", 227.769, 469.452)}
                                            {renderProvinceName("Kwango", 227.769, 469.452)}
                                        </g>
                                    </a>
                                </g>
                                <a href="#" id="region-9" title="Kinshasa" onClick={() => handleTitle("Kinshasa")}>
                                    <g>
                                        <path
                                            id="CD-KN"
                                            className={`${title === "Kinshasa" ? "fill-active" : "fill"
                                                }`}
                                        ...
                                        ></path>
                                        {renderCircleWithTooltip("Kinshasa", 141.945, 376.608)}
                                        {renderProvinceName("Kinshasa", 141.945, 376.608)}
                                    </g>
                                </a>
                                <a href="#" id="region-10" title="Kasaï Oriental"
                                    onClick={() => handleTitle("Kasaï Oriental")}>
                                    <g>
                                        <path
                                            id="CD-KO"
                                            className={`${title === "Kasaï-Oriental" ? "fill-active" : "fill"
                                                }`}
                                        ...
                                        ></path>
                                        {renderCircleWithTooltip("Kasaï-Oriental", 433.338, 440.978)}
                                        {renderProvinceName("Kasaï-Oriental", 433.338, 440.978)}
                                    </g>
                                </a>
                                <a href="#" id="region-11" title="Kasaï" onClick={() => handleTitle("Kasaï")}>
                                    <g>
                                        <path
                                            id="CD-KS"
                                            className={`${title === "Kasaï" ? "fill-active" : "fill"
                                                }`}
                                        ...
                                        ></path>
                                        {renderCircleWithTooltip("Kasaï", 348.657, 380.715)}
                                        {renderProvinceName("Kasaï", 348.657, 380.715)}
                                    </g>
                                </a>
                                <a
                                    href="#"
                                    id="region-12"
                                    title="Kwilu"
                                    onClick={() => handleTitle("Kwilu")}
                                >
                                    <g>
                                        <path
                                            id="CD-KU"
                                            className={`${title === "Kwilu" ? "fill-active" : "fill"
                                                }`}
                                        ...
                                        ></path>
                                        {renderCircleWithTooltip("Kwilu", 241.008, 379.954)}
                                        {renderProvinceName("Kwilu", 241.008, 379.954)}
                                    </g>
                                </a>
                                <a href="#" id="region-13" title="Lualaba" onClick={() => handleTitle("Lualaba")}>
                                    <g>
                                        <path
                                            id="CD-LB"
                                            className={`${title === "Lualaba" ? "fill-active" : "fill"
                                                }`}
                                        ...
                                        ></path>
                                        {renderCircleWithTooltip("Lualaba", 421.597, 574.156)}
                                        {renderProvinceName("Lualaba", 421.597, 574.156)}
                                    </g>
                                </a>
                                <a href="#" id="region-14" title="Kasaï-Central"
                                    onClick={() => handleTitle("Kasaï-Central")}>
                                    <g>
                                        <path
                                            id="CD-LL"
                                            className={`${title === "Kasaï-Central" ? "fill-active" : "fill"
                                                }`}
                                        ...
                                        ></path>
                                        {renderCircleWithTooltip("Kasaï-Central", 397.705, 466.011)}
                                        {renderProvinceName("Kasaï-Central", 397.705, 466.011)}
                                    </g>
                                </a>
                                <a href="#" id="region-15" title="Lomami" onClick={() => handleTitle("Lomami")}>
                                    <g>
                                        <path
                                            id="CD-LM"
                                            className={`${title === "Lomami" ? "fill-active" : "fill"
                                                }`}
                                        ...
                                        ></path>
                                        {renderCircleWithTooltip("Lomami", 494.299, 437.652)}
                                        {renderProvinceName("Lomami", 494.299, 437.652)}
                                    </g>
                                </a>
                                <a
                                    href="#"
                                    id="region-16"
                                    title="Mai-Ndombe"
                                    onClick={() => handleTitle("Mai-Ndombe")}
                                >
                                    <g>
                                        <path
                                            id="CD-MA"
                                            className={`${title === "Mai-Ndombe" ? "fill-active" : "fill"
                                                }`}
                                        ...
                                        ></path>
                                        {renderCircleWithTooltip("Mai-Ndombe", 238.657, 300.715)}
                                        {renderProvinceName("Mai-Ndombe", 238.657, 300.715)}
                                    </g>
                                </a>
                                <a href="#" id="region-17" title="Maniema" onClick={() => handleTitle("Maniema")}>
                                    <g>
                                        <path
                                            id="CD-MN"
                                            className={`${title === "Maniema" ? "fill-active" : "fill"
                                                }`}
                                        ...
                                        ></path>
                                        {renderCircleWithTooltip("Maniema", 530.881, 321.549)}
                                        {renderProvinceName("Maniema", 530.881, 321.549)}
                                    </g>
                                </a>
                                <a href="#" id="region-18" title="Mongala" onClick={() => handleTitle("Mongala")}>
                                    <g>
                                        <path
                                            id="CD-MO"
                                            className={`${title === "Mongala" ? "fill-active" : "fill"
                                                }`}
                                        ...
                                        ></path>
                                        {renderCircleWithTooltip("Mongala", 349.6, 128.154)}
                                        {renderProvinceName("Mongala", 349.6, 128.154)}
                                    </g>
                                </a>
                                <a href="#" id="region-19" title="Nord-Kivu" onClick={() => handleTitle("Nord-Kivu")}>
                                    <g>
                                        <path
                                            id="CD-NK"
                                            className={`${title === "Nord-Kivu" ? "fill-active" : "fill"
                                                }`}
                                        ...
                                        ></path>
                                        {renderCircleWithTooltip("Nord-Kivu", 636.057, 217.52)}
                                        {renderProvinceName("Nord-Kivu", 636.057, 217.52)}
                                    </g>
                                </a>
                                <a
                                    href="#"
                                    id="region-20"
                                    title="Nord-Ubangi"
                                    onClick={() => handleTitle("Nord-Ubangi")}
                                >
                                    <g>
                                        <path
                                            id="CD-NU"
                                            className={`${title === "Nord-Ubangi" ? "fill-active" : "fill"
                                                }`}
                                        ...
                                        ></path>
                                        {renderCircleWithTooltip("Nord-Ubangi", 354.701, 57.163)}
                                        {renderProvinceName("Nord-Ubangi", 354.701, 57.163)}
                                    </g>
                                </a>
                                <a href="#" id="region-21" title="Sud-Kivu" onClick={() => handleTitle("Sud-Kivu")}>
                                    <g>
                                        <path
                                            id="CD-SK"
                                            className={`${title === "Sud-Kivu" ? "fill-active" : "fill"
                                                }`}
                                        ...
                                        ></path>
                                        {renderCircleWithTooltip("Sud-Kivu", 626.057, 327.52)}
                                        {renderProvinceName("Sud-Kivu", 626.057, 327.52)}
                                    </g>
                                </a>
                                <a href="#" id="region-22" title="Sankuru" onClick={() => handleTitle("Sankuru")}>
                                    <g>
                                        <path
                                            id="CD-SN"
                                            className={`${title === "Sankuru" ? "fill-active" : "fill"
                                                }`}
                                        ...
                                        ></path>
                                        {renderCircleWithTooltip("Sankuru", 428.779, 341.217)}
                                        {renderProvinceName("Sankuru", 428.779, 341.217)}
                                    </g>
                                </a>
                                <a
                                    href="#"
                                    id="region-23"
                                    title="Sud-Ubangi"
                                    onClick={() => handleTitle("Sud-Ubangi")}
                                >
                                    <g>
                                        <path
                                            id="CD-SU"
                                            className={`${title === "Sud-Ubangi" ? "fill-active" : "fill"
                                                }`}
                                        ...
                                        ></path>
                                        {renderCircleWithTooltip("Sud-Ubangi", 270.723, 88.832)}
                                        {renderProvinceName("Sud-Ubangi", 270.723, 88.832)}
                                    </g>
                                </a>
                                <a href="#" id="region-24" title="Tanganyika" onClick={() => handleTitle("Tanganyika")}>
                                    <g>
                                        <path
                                            id="CD-TG"
                                            className={`${title === "Tanganyika" ? "fill-active" : "fill"
                                                }`}
                                        ...
                                        ></path>
                                        {renderCircleWithTooltip("Tanganyika", 599.698, 458.104)}
                                        {renderProvinceName("Tanganyika", 599.698, 458.104)}
                                    </g>
                                </a>
                                <a href="#" id="region-25" title="Tshopo" onClick={() => handleTitle("Tshopo")}>
                                    <g>
                                        <path
                                            id="CD-TO"
                                            className={`${title === "Tshopo" ? "fill-active" : "fill"
                                                }`}
                                        ...
                                        ></path>
                                        {renderCircleWithTooltip("Tshopo", 500.445, 181.449)}
                                        {renderProvinceName("Tshopo", 500.445, 181.449)}
                                    </g>
                                </a>
                                <a href="#" id="region-26" title="Tshuapa" onClick={() => handleTitle("Tshuapa")}>
                                    <g>
                                        <path
                                            id="CD-TP"
                                            className={`${title === "Tshuapa" ? "fill-active" : "fill"
                                                }`}
                                        ...
                                        ></path>
                                        {renderCircleWithTooltip("Tshuapa", 359.6, 230.154)}
                                        {renderProvinceName("Tshuapa", 359.6, 230.154)}
                                    </g>
                                </a>
